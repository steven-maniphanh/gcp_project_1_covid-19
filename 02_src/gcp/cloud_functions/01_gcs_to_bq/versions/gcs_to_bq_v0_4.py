...
def gcs_to_bq(event, context):
    ...
    try:
        ...
        # Step 2.1: Validate & normalize expected columns
        df_data.columns = df_data.columns.str.strip().str.lower().str.replace(' ', '_')  # Normalize CSV column names
        expected_columns = ["country", "region", "date", "new_cases", "total_cases", "new_deaths", "total_deaths"]
        missing_columns = set(expected_columns) - set(df_data.columns)
        if missing_columns:
            raise ValueError(f"Missing columns in input file: {missing_columns}")
        
        ## Step 2.2: Add ingestion_timestamp column on data loaded in BQ table
        df_data["ingestion_timestamp"] = datetime.now(timezone.utc) 
